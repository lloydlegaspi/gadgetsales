...
export function WalletStatus({ address, isConnecting, error, onConnect, onDisconnect }: WalletStatusProps) {
  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-red-700">
        <p className="font-semibold">Wallet Error</p>
        <p className="text-sm">{error}</p>
        <button onClick={onConnect} className="mt-2 text-sm underline hover:no-underline">
